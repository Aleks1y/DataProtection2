...
//инициализируем начальные 8 хэш-значений
...
//считаем фрагмент размером 512 бит
static void sha256_calc_chunk(struct sha256_buff* buff, const uint8_t* chunk) {
    ...
    //считаем оставшиеся элементы массива
    for (i = 16; i < 64; ++i) {
...
void sha256_update(struct sha256_buff* buff, const void* data, size_t size) {
    ...
    buff->data_size += size;
    // если в буффере остались данные, объединяем с новыми
    if (buff->chunk_size > 0 && size + buff->chunk_size >= 64) {
    ...
    // сохраняем оставшиеся данные в буффере
    memcpy(buff->last_chunk + buff->chunk_size, ptr, size);
    buff->chunk_size += size;
}

// считаем хэш для последнего фрагмента данных
void sha256_finalize(struct sha256_buff* buff) {
    ...
    // если места для дополнения до 512 бит недостаточно
    if (buff->chunk_size > 56) {
        sha256_calc_chunk(buff, buff->last_chunk);
        memset(buff->last_chunk, 0, 64);
    }

    // в конец фрагмента добавляем длину входного сообщения в двоичном формате
    ...
    // считаем хэш последнего фрагмента
    sha256_calc_chunk(buff, buff->last_chunk);
}

// преобразование структуры в массив - 32 байта
...
// преобразование структуры в число шестнадцатиричной системы (итоговое значение хэша)
...
// получаем финальный хэш из структуры
